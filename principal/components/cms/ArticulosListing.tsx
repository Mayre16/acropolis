...
export function ArticulosListing() {
  ...
  const cmsOnly = edit?.ready
    ? edit.items.filter((a) => !isSeedArticulo(a.slug))
    : [];
  ...
  return (
    <>
      ...
      <ul className="mt-12 grid items-start gap-7 sm:grid-cols-2 lg:grid-cols-3">
        {rest.map((a) => (
          <li key={a.slug}>
            <CardWrap
              slug={a.slug}
