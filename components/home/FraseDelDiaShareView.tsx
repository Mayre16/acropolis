...
export function FraseDelDiaShareView({
  ...
  initial: (CmsFraseDelDia & { dayIndex?: number }) | null;
  dayIndex?: number;
}) {
  const live = useCmsFrasesDelDia();
  const effectiveDayIndex = initial?.dayIndex ?? dayIndex;
  const dia = getDiaFromIndex(effectiveDayIndex);
  
  const liveMatch = live[effectiveDayIndex];
  const frase = (liveMatch?.src?.trim() ? liveMatch : null) ?? initial;
  const src = frase?.src
  ...
  return (
    <section className="mx-auto max-w-lg px-5 py-12 sm:px-8 sm:py-16">
      ...
      <p className="mx-auto mt-3 max-w-md text-center text-sm text-na-muted">
        {fraseDelDiaShareText(effectiveDayIndex)}
      </p>
      ...
      <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
        <button
          type="button"
          onClick={() => void shareFraseDelDiaLink(effectiveDayIndex)}
