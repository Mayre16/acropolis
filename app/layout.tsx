      ...
      <head>
        ...
        <script
          dangerouslySetInnerHTML={{
            ...
            )};if(!api)return;var origin=api.replace(/\\/api$/i,"");var u=api+"/content/acropolis/published";var slot=window.__acropolisCmsPublished=window.__acropolisCmsPublished||{};if(slot.promise)return;slot.promise=new Promise(function(resolve){function boot(){fetch(u,{cache:"no-store"}).then(function(r){return r.ok?r.json():null}).then(function(d){slot.doc=d;try{var src=d&&d.sections&&d.sections.homeHero&&d.sections.homeHero.background&&d.sections.homeHero.background.src;if(src&&src.indexOf("/uploads/")!==-1){var href=src.indexOf("http")===0?src:(origin+src);var l=document.createElement("link");l.rel="preload";l.as="image";l.href=href;document.head.appendChild(l)}}catch(e){}resolve(d)}).catch(function(){resolve(null)})}if(window.requestIdleCallback)requestIdleCallback(boot,{timeout:2500});else setTimeout(boot,1)})}catch(e){}})();`,
          }}
        />
      </head>
      <body
